...
class HKAuthorizer {
    static func authorizeHealthKit(completion: @escaping (Bool, Error?) -> Swift.Void) {
        ...
        let types: Set = [HKObjectType.electrocardiogramType(), HKObjectType.quantityType(forIdentifier: HKQuantityTypeIdentifier.bodyMass)!, HKQuantityType.quantityType(forIdentifier: HKQuantityTypeIdentifier.height)!]
